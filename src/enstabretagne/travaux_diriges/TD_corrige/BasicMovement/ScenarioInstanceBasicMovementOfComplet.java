...
public class ScenarioInstanceBasicMovementOfComplet implements IScenarioInstance {
	...
	public IScenario getScenarioInstance() {
		...
		bsf.getNavires().put(new EntityNavireFeature("NavireF", 5, 3, Color.BROWN, feat),
				new EntityNavireInit("Navire Observation 2", msi));
		...
		for (i = 0; i < NbSphere; i++) {
			...
			mstArtefact = new MovableState(new Point3D(X, Y, Z), Point3D.ZERO, Point3D.ZERO, Point3D.ZERO, Point3D.ZERO,
					Point3D.ZERO);
			msiArtefact = new EntityMouvementSequenceurInit("MSI", mstArtefact, 0, 0, 0, 0, positionsCles, 0);
			bsf.getArtefactes().put(new EntityArtefactFeature("B1", 5, 1, Color.RED, mssArtefact, Sphere),
					new EntityArtefactInit("B" + i, msiArtefact));
		...
		for (i = 0; i < NbBox; i++) {
			...
			mstArtefact = new MovableState(new Point3D(X, Y, Z), Point3D.ZERO, Point3D.ZERO, Point3D.ZERO, Point3D.ZERO,
					Point3D.ZERO);
			msiArtefact = new EntityMouvementSequenceurInit("MSI", mstArtefact, 0, 0, 0, 0, positionsCles, 0);
			bsf.getArtefactes().put(new EntityArtefactFeature("Cu1", 5, 1, Color.YELLOW, mssArtefact, Cube),
					new EntityArtefactInit("Cu" + i, msiArtefact));
		...
		for (i = 0; i < NbCylinder; i++) {
			...
			mstArtefact = new MovableState(new Point3D(X, Y, Z), Point3D.ZERO, Point3D.ZERO, Point3D.ZERO, Point3D.ZERO,
					Point3D.ZERO);
			msiArtefact = new EntityMouvementSequenceurInit("MSI", mstArtefact, 0, 0, 0, 0, positionsCles, 0);
			bsf.getArtefactes().put(new EntityArtefactFeature("Cy1", 5, 1, Color.GREEN, mssArtefact, Cylinder),
					new EntityArtefactInit("Cy" + i, msiArtefact));
		...
		for (i = 0; i < 1; i++) {
			...
			bsf.getArtefactes().put(new EntityArtefactFeature("Objet1", 5, 1, Color.BLACK, mssArtefact, Cube),
					new EntityArtefactInit("Objet" + i, msiArtefact));
